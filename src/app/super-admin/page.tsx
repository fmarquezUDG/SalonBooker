...
export default function SuperAdminDashboard() {
  ...
  // Función para aprobar o rechazar salones
  const handleAprobarSalon = async (salonId: string, aprobar: boolean) => {
    try {
      const response = await fetch(`/api/admin/salones/${salonId}/aprobar`, {
        method: 'PATCH',
        headers: {
      ...
      if (response.ok) {
        ...
        setSalones(prevSalones => 
          prevSalones.map(salon => 
            salon.id === salonId 
              ? { ...salon, estado: aprobar ? 'activo' : 'inactivo' }
              : salon
          )
        );
        
        alert(aprobar ? '✅ Salón aprobado exitosamente' : '❌ Salón rechazado');
                  ...
                  <div key={salon.id} className="bg-white/80 backdrop-blur-sm rounded-2xl md:rounded-3xl p-4 md:p-8 border border-purple-100 shadow-lg hover:shadow-2xl hover:scale-105 transition-all duration-500">
                    ...
                    {salon.estado === 'pendiente' && (
                      <div className="mb-4 flex gap-2">
                        <button
                          onClick={() => handleAprobarSalon(salon.id, true)}
                          className="flex-1 flex items-center justify-center space-x-2 px-4 py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-semibold transition-all"
                        >
                          <CheckCircle className="w-4 h-4" />
                          <span>Aprobar</span>
                        </button>
                        <button
                          onClick={() => handleAprobarSalon(salon.id, false)}
                          className="flex-1 flex items-center justify-center space-x-2 px-4 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-semibold transition-all"
                        >
